...
fn bench_pixel_diff_algorithm(c: &mut Criterion) {
    ...
    for (width, height, size_name) in frame_sizes {
        ...
        group.bench_with_input(
            ...
            |b, frame_data| {
                b.iter(|| {
                    service.detect_motion_from_frame(frame_data, width, height).unwrap();
                });
            },
        );
    }
...
fn bench_algorithm_comparison(c: &mut Criterion) {
    ...
    let _ = pixel_service.detect_motion_from_bytes(&frame1_bytes).unwrap();
    
    group.bench_function("pixel_diff_320x240", |b| {
        b.iter(|| {
            pixel_service.detect_motion_from_bytes(&frame2_bytes).unwrap();
        });
    });
    ...
    #[cfg(feature = "heavy_opencv")]
    {
        ...
        for _ in 0..5 {
            let _ = mog2_service.detect_motion_from_bytes(&frame1_bytes).unwrap();
        }
        
        group.bench_function("mog2_320x240", |b| {
            b.iter(|| {
                mog2_service.detect_motion_from_bytes(&frame2_bytes).unwrap();
            });
        });
    }
...
fn create_identical_frames(width: u32, height: u32) -> (gl_vision::image::GrayImage, gl_vision::image::GrayImage) {
    let frame1 = create_test_frame_with_motion(width, height, 0, 0, 0, 0, 64);
    let frame2 = frame1.clone();
    (frame1, frame2)
}

fn create_small_motion_frames(width: u32, height: u32) -> (gl_vision::image::GrayImage, gl_vision::image::GrayImage) {
    let frame1 = create_test_frame_with_motion(width, height, 0, 0, 0, 0, 64);
    let frame2 = create_test_frame_with_motion(width, height, width/4, height/4, 20, 20, 128);
    (frame1, frame2)
}

fn create_large_motion_frames(width: u32, height: u32) -> (gl_vision::image::GrayImage, gl_vision::image::GrayImage) {
    let frame1 = create_test_frame_with_motion(width, height, 0, 0, 0, 0, 64);
    let frame2 = create_test_frame_with_motion(width, height, 50, 50, width/3, height/3, 200);
    (frame1, frame2)
}

fn create_multi_object_frames(width: u32, height: u32) -> (gl_vision::image::GrayImage, gl_vision::image::GrayImage) {
