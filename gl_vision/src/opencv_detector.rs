...
pub struct OpenCvDetector {
    config: MotionConfig,
    mog2: opencv::core::Ptr<dyn BackgroundSubtractorMOG2>,
    initialized: bool,
}
...
impl OpenCvDetector {
    ...
    fn count_foreground_pixels(&self, mask: &Mat) -> Result<u32> {
        let non_zero = opencv::core::count_non_zero(mask)
            .map_err(|e| gl_core::Error::Database(format!("Failed to count non-zero pixels: {}", e)))?;
        Ok(non_zero as u32)
    }
    ...
    fn post_process_mask(&self, mask: &mut Mat) -> Result<()> {
        ...
        ).map_err(|e| gl_core::Error::Database(format!("Failed to create morphological kernel: {}", e)))?;
        ...
        ).map_err(|e| gl_core::Error::Database(format!("Failed to apply morphological opening: {}", e)))?;
        ...
        opencv::imgproc::morphology_ex(
            ...
            opencv::core::BORDER_CONSTANT,
            opencv::imgproc::morphology_default_border_value().unwrap(),
        ).map_err(|e| gl_core::Error::Database(format!("Failed to apply morphological closing: {}", e)))?;
...
impl MotionDetector for OpenCvDetector {
    fn detect_motion(&mut self, current_frame: &[u8], frame_width: u32, frame_height: u32) -> Result<MotionResult> {
        ...
        self.mog2.apply(&frame_mat, &mut foreground_mask, -1.0)
        ...
        let motion_detected = changed_pixels >= self.config.min_change_area 
            && change_ratio >= self.config.threshold;
        ...
        let confidence = if motion_detected {
            ...
            (area_confidence * 0.5 + threshold_confidence * 0.5).min(0.99).max(0.8)
        } else {
    ...
    fn reset(&mut self) -> Result<()> {
        ...
        self.mog2 = opencv::video::create_background_subtractor_mog2(
            500,
            16.0,
            true,
        ).map_err(|e| gl_core::Error::Database(format!("Failed to recreate MOG2 detector: {}", e)))?;
...
impl OpenCvDetector {
    pub fn new(_config: crate::MotionConfig) -> Result<Self> {
        Err(gl_core::Error::Config("OpenCV not available - heavy_opencv feature not enabled".to_string()))
    }
}
...
mod tests {
    ...
    fn test_opencv_detector_with_motion() {
        ...
        for _ in 0..10 {
            let _ = detector.detect_motion(background.as_raw(), 100, 100).unwrap();
        }
        
        // Now introduce motion
        let motion_frame = create_test_frame_with_motion(100, 100, 20, 20, 30, 30, 200);
        let result = detector.detect_motion(motion_frame.as_raw(), 100, 100).unwrap();
