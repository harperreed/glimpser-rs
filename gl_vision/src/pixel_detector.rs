...
impl PixelDiffDetector {
    ...
    fn apply_noise_reduction(&self, changed_pixels: u32, total_pixels: u32) -> (bool, f64) {
        ...
        let confidence = if motion_detected {
            (area_confidence * 0.6 + threshold_confidence * 0.4).min(0.99).max(0.7)
        } else {
            (change_ratio / self.config.threshold * 0.5).min(0.6)
        };
        
        debug!("Motion analysis: changed_pixels={}, change_ratio={:.3}, confidence={:.3}, motion={}",
...
impl MotionDetector for PixelDiffDetector {
    fn detect_motion(&mut self, current_frame: &[u8], frame_width: u32, frame_height: u32) -> Result<MotionResult> {
        ...
        let (changed_pixels, change_ratio) = self.calculate_pixel_diff(current_frame, previous_frame);
        
        // Apply noise reduction and determine motion
        let (motion_detected, confidence) = self.apply_noise_reduction(changed_pixels, total_pixels);
...
mod tests {
    ...
    fn test_pixel_diff_different_thresholds() {
        ...
        let _ = detector_high.detect_motion(frame1.as_raw(), 100, 100).unwrap();
        let _ = detector_low.detect_motion(frame1.as_raw(), 100, 100).unwrap();
        
        let result_high = detector_high.detect_motion(frame2.as_raw(), 100, 100).unwrap();
        let result_low = detector_low.detect_motion(frame2.as_raw(), 100, 100).unwrap();
