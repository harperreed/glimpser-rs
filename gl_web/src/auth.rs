...
mod tests {
    ...
    fn test_password_hash_and_verify() {
        ...
        let is_valid = PasswordAuth::verify_password("wrong_password", &hash).expect("Should verify");
        assert!(!is_valid);
    }
    ...
    fn test_jwt_create_and_verify() {
        ...
        // Create token
        let token = JwtAuth::create_token(user_id, email, role, secret).expect("Should create token");
