...
use gl_cap::{
    ...
    validation::Validate,
    Alert, Category, Certainty, MsgType, ResponseType, Scope, Severity, Status, Urgency,
};
...
fn test_parse_test_alert_golden_file() {
    ...
    assert_eq!(info.headline, Some("TEST ALERT - NO ACTION REQUIRED".to_string()));
...
fn test_profiles_xml_serialization() {
    let profiles = vec![
        ...
        ("severe_weather", AlertProfiles::severe_weather("weather.example.org")),
        ("fire", AlertProfiles::fire_alert("fire.example.org")),
        ("public_safety", AlertProfiles::public_safety("safety.example.org")),
    ];
...
fn test_timestamp_serialization() {
    let test_time: DateTime<Utc> = "2023-06-15T14:30:00Z".parse().expect("Should parse timestamp");
...
fn test_serialization_performance() {
    ...
    assert!(elapsed.as_secs() < 1, "100 roundtrips should complete in under 1 second, took: {:?}", elapsed);
}