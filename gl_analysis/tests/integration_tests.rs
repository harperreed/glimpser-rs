...
use gl_analysis::{
    AnalysisService, AnalysisConfig, ProcessorInput, ProcessorContext, EventSeverity,
    rule_engine::{RuleSet, Rule, Condition, Action, ConditionType, ComparisonOperator, DeduplicationConfig, QuietHoursConfig},
};
...
fn create_test_frame() -> Bytes {
    ...
    Bytes::from(vec![0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46])
}
...
async fn test_motion_burst_deduplication() {
    ...
    for i in 0..5 {
        ...
        motion_events += events.iter().filter(|e| e.event_type == "motion_detected").count();
    ...
    println!("Total motion events: {} out of {} total events", motion_events, total_events);
    ...
    assert!(motion_events < 5, "Expected deduplication to reduce motion events, got {}", motion_events);
    assert!(motion_events >= 1, "At least first motion event should pass through");
}
...
async fn test_time_window_rules() {
    ...
    println!("Business hours test passed: {} events with notifications suppressed", events.len());
}
...
async fn test_severity_based_rules() {
    ...
    let escalated_events: Vec<_> = events.iter()
        .filter(|e| e.metadata.get("escalated").is_some())
        .collect();
    
    println!("Found {} escalated events out of {} total", escalated_events.len(), events.len());
...
async fn test_quiet_hours() {
    ...
    for event in &events {
        assert!(!event.should_notify, 
               "Event {} should not notify during quiet hours", event.event_type);
    }
    
    println!("Quiet hours test passed: {} events with notifications suppressed", events.len());
}
...
async fn test_event_count_rules() {
    ...
    let burst_rule = Rule {
        ...
        actions: vec![
            Action::SetSeverity { severity: EventSeverity::Critical },
            Action::AddMetadata { 
    ...
    for i in 0..4 {
        ...
        if i >= 2 { // After we have 3+ events in history
            let burst_events: Vec<_> = events.iter()
                .filter(|e| e.metadata.get("burst_detected").is_some())
                .collect();
            
            if !burst_events.is_empty() {
                println!("Burst alert triggered after {} iterations", i + 1);
                assert!(!burst_events.is_empty(), "Expected burst alert to be triggered");
...
async fn test_full_processor_chain() {
    let config = AnalysisConfig {
        ...
        processor_configs: {
            ...
            configs.insert("ai_description".to_string(), serde_json::json!({
                "motion_only": false,
                "detail_level": "high",
                "focus": "security"
            }));
            configs.insert("summary".to_string(), serde_json::json!({
                "min_events": 1,
                "max_length": 150,
    ...
    for event in &events {
        println!("  - {}: {} ({})", event.event_type, event.description, event.processor_name);
    }
    
    // Should have events from different processors
    let processor_names: std::collections::HashSet<_> = events.iter()
...
async fn test_rule_priority() {
    ...
    let low_priority_rule = Rule {
        ...
        actions: vec![Action::SetSeverity { severity: EventSeverity::High }],
        enabled: true,
        priority: 10,
    };
    ...
    let high_priority_rule = Rule {
        ...
        actions: vec![Action::SetSeverity { severity: EventSeverity::Critical }],
        enabled: true,
    ...
    let motion_events: Vec<_> = events.iter()
...
async fn test_config_updates() {
    ...
    let processor_types2: std::collections::HashSet<_> = events2.iter()
        .map(|e| e.processor_name.as_str())
        .collect();
...
async fn test_error_resilience() {
    ...
    println!("Second analysis after errors generated {} events", events2.len());
}