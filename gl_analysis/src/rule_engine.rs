...
use crate::{ProcessorInput, AnalysisEvent, EventSeverity};
...
pub enum ConditionType {
    ...
    /// Severity comparison
    Severity { operator: ComparisonOperator, value: EventSeverity },
    /// Confidence threshold
    Confidence { operator: ComparisonOperator, value: f64 },
    /// Time-based condition
    TimeWindow { start: String, end: String, days: Vec<u8> },
...
impl RuleEngine {
    ...
    pub async fn apply_rules(
        ...
        debug!("Applying {} rules to {} events", rule_set.rules.len(), events.len());
        ...
        for mut event in events {
            ...
            for rule in &rules {
                ...
                if self.evaluate_rule_conditions(rule, &event, input).await? {
                    debug!("Rule '{}' matched for event {}", rule.name, event.event_type);
        ...
        debug!("Rule engine kept {} out of {} events", events_to_keep.len(), events_count);
    ...
    async fn evaluate_rule_conditions(
        ...
        for condition in &rule.conditions {
            if !self.evaluate_condition(&condition.condition_type, event).await? {
                return Ok(false);
            }
        }
    ...
    async fn evaluate_condition(
        ...
        match condition {
            ...
            ConditionType::EventCount { event_type, count, operator, window_minutes } => {
                let matching_count = self.count_recent_events(event_type.as_deref(), *window_minutes);
                Ok(self.compare_numeric(matching_count as f64, operator, *count as f64))
            },
            
            ConditionType::Metadata { field, operator, value } => {
    ...
    async fn apply_action(&self, action: &Action, event: &mut AnalysisEvent) -> Result<bool> {
        match action {
            ...
            Action::SetSeverity { severity } => {
                debug!("Changing severity from {:?} to {:?}", event.severity, severity);
            ...
            Action::SetNotificationTemplate { template } => {
                ...
                event.metadata.insert("notification_template".to_string(), template.clone().into());
                Ok(true)
            },
    ...
    fn check_deduplication(&self, event: &AnalysisEvent, config: &DeduplicationConfig) -> bool {
        ...
        let has_recent_duplicate = self.event_history.iter()
        ...
        if has_recent_duplicate {
            debug!("Deduplicating event {} with key: {}", event.event_type, dedup_key);
            false
        } else {
            true
        }
    ...
    fn apply_quiet_hours(&self, event: &mut AnalysisEvent, config: &QuietHoursConfig) {
        if self.is_in_time_window(&event.timestamp, &config.start_time, &config.end_time, &config.days) {
    ...
    fn compare_severity(&self, event_severity: &EventSeverity, op: &ComparisonOperator, target: &EventSeverity) -> bool {
        match op {
    ...
    fn is_in_time_window(&self, timestamp: &DateTime<Utc>, start: &str, end: &str, days: &[u8]) -> bool {
    ...
    fn compare_metadata_value(&self, event_value: Option<&serde_json::Value>, op: &ComparisonOperator, target: &serde_json::Value) -> bool {
