...
use cron::Schedule;
use gl_core::{Result, Error};
use std::str::FromStr;

/// Calculate the next N run times for a cron expression
pub fn calculate_next_runs(cron_expr: &str, _from: DateTime<Utc>, count: usize) -> Result<Vec<DateTime<Utc>>> {
...
mod tests {
    ...
    fn test_describe_next_run() {
        ...
        assert!(description.contains("in") || description.contains("minutes") || description.contains("seconds"));
    }
