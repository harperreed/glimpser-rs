...
pub async fn run(spec: CommandSpec) -> Result<CommandResult> {
    ...
    let stdout_task = capture_output(
        stdout,
        spec.stdout_limit.unwrap_or(DEFAULT_OUTPUT_LIMIT),
        "stdout",
    );
    let stderr_task = capture_output_with_logging(
        stderr,
        spec.stderr_limit.unwrap_or(DEFAULT_OUTPUT_LIMIT),
        "stderr",
    );
    ...
    let execution_result = timeout(spec.timeout, async {
        ...
        let (status_result, (stdout_output, stdout_truncated), (stderr_output, stderr_truncated)) =
    ...
    let (status, stdout_output, stdout_truncated, stderr_output, stderr_truncated, timed_out) =
        match execution_result {
            ...
            Err(_) => {
                warn!(
                    timeout_secs = spec.timeout.as_secs(),
                    "Command timed out, initiating graceful shutdown"
                );
                ...
                match kill_result {
                    ...
                    _ => {
                        error!(
                            "Command did not terminate gracefully, process may still be running"
                        );
...
mod tests {
    ...
    async fn test_command_timeout() {
        ...
        let result = run(spec)
            .await
            .expect("Command should complete with timeout");
    ...
    async fn test_output_truncation() {
        ...
        let spec = CommandSpec::new("echo".into()).args([&large_text]);
